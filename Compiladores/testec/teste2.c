...
int main(void)
{
 ...
 char *M[7][10];
 char *cifra = "$";
 int linha = 0 ,coluna = 0, lib = 1, para = 0, erro = 0;
    ...
    while(lib != 2) {
        if((!(strcmp(topo,"E"))) && (exp[v] == 69)){
            ...
            if(linha == 0 || coluna == 0){
                ...
                printf("topo: %c\n",topo[0]);
                printf("exp: %c\n",exp[v]);
                printf("Erro Branco, Pula <%c> na exp\n",exp[v]);
                exibe(PILHA);
                erro++;
                lib++;
            } else if (M[linha][coluna] == "s") {
                printf("Erro Sinc, Tira <%c> da Pilha\n",topo[0]);
                if(tam < 3) {
                  printf("topo: %c\n",topo[0]);
                  printf("exp: %c\n",exp[v]);
                  printf("Erro de tamanho, Pula <%c> na exp\n",exp[v]);
                  exibe(PILHA);
                  lib++;
                } else {
                  printf("topo: %c\n",topo[0]);
